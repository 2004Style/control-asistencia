...
@Component({
  selector: 'app-attendances-edit-admin',
  imports: [
    NgIf,
    ReactiveFormsModule,
    MatFormFieldModule,
  ...
  templateUrl: './attendances-edit-admin.component.html',
  styleUrl: './attendances-edit-admin.component.css',
})
export class AttendancesEditAdminComponent {
  form: FormGroup;
  id: number;
  isEdit: boolean;
  ...
  ngOnInit(): void {
    this._service
      ...
      .toPromise()
      .then(() => {
        const model = new AttendancesDtoTs();
  ...
  operate() {
    const model = new AttendancesDtoTs(this.form.value);
