...
@Component({
  selector: 'app-tardiness-edit-admin',
  imports: [
    NgIf,
    ReactiveFormsModule,
    MatFormFieldModule,
  ...
  templateUrl: './tardiness-edit-admin.component.html',
  styleUrl: './tardiness-edit-admin.component.css',
})
export class TardinessEditAdminComponent {
  form: FormGroup;
  id: number;
  isEdit: boolean;
  ...
  ngOnInit(): void {
    this._service
      ...
      .toPromise()
      .then(() => {
        const model = new TardinessDtoTs();
  ...
  operate() {
    const model = new TardinessDtoTs(this.form.value);
